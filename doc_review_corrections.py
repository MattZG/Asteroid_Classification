...
correcciones = {
    ...
    "02_Calidad de datos": [
        ("Las variables de *diameter* y *diameter_sigma* son tiene relación con el diametro del asteroide. Lo que son variable numericas, pero son reconocidas como varaible categoricas (object), toca cambiar su tipo",
         "Las variables *diameter* y *diameter_sigma* están relacionadas con el diámetro del asteroide. Son variables numéricas, pero se reconocen como variables categóricas (object); es necesario cambiar su tipo."),
        ("Aplicar este cambio puede inducir nulos en el dataset, asi que toca revisar eso",
         "Aplicar este cambio puede inducir valores nulos en el dataset, así que es necesario revisar esto."),
        ("Se completan estos nuevos valores nulos con la mediana, para no alterar la distibuci\u00f3n de los datos",
         "Los nuevos valores nulos se completan con la mediana para no alterar la distribución de los datos."),
        ("Dado que solo tiene una variable categorica, no es necesario separar el dataset por tipos de varaibles.",
         "Dado que solo hay una variable categórica, no es necesario separar el dataset por tipos de variables."),
    ],
    # 03_EDA.ipynb
    "03_EDA": [
...
# Procesar notebooks
for prefix, replacements in correcciones.items():
    if prefix == "02_Calidad de datos":
        nb_path = Path('03_Notebooks/01_Desarrollo/02_Calidad de datos.ipynb')
    else:
    ...
    for old, new in replacements:
        if old in content:
            ...
            print(f"✓ Corregido en {nb_path.name}: {old[:50]}...")
        else:
